...
export default function Header() {
  ...
  const fetchPublicBeasiswaPeriods = async () => {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `API returned ${response.status} from ${baseURL}/public/beasiswa-periods`,
        );
        return { data: [] };
      }
  ...
  useEffect(() => {
    ...
    // Poll every 5 minutes (300 seconds) instead of 30 seconds
    const intervalId = setInterval(checkPeriodsStatus, 300000);
  ...
  const hasActivePeriod = () => {
    ...
    const hasActive = beasiswaPeriods.some(
  ...
  // tambah
  // Get logo URL - using local logo file
  const getLogoUrl = () => {
    return "/storage/bersekolah.png";
  };
    ...
    <Navbar isBordered isMenuOpen={isMenuOpen} onMenuOpenChange={setIsMenuOpen}>
      ...
      <NavbarContent className="hidden gap-4 md:flex" justify="center">
        ...
        <NavbarItem className="hidden md:flex xl:hidden">
          <Dropdown>
            <DropdownTrigger>
              <Button variant="light" className="flex items-center gap-1">
            ...
            <DropdownMenu aria-label="Menu Lainnya">
              {navItems.slice(4).map((item, index) => (
                <DropdownItem
                  key={index}
                  className={isActive(item.href) ? "text-[#406386]" : ""}
                >
                  <Link
      ...
      <NavbarContent className="hidden md:flex" justify="end">
        {isLoggedIn ? (
          ...
          <>
            ...
            {/* Only show the register button if there is an active period */}
            {hasActivePeriod() && (
              <NavbarItem className="hidden lg:flex">
                <Link
                  href="/daftar"
