    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.token) {
          ...
          if (typeof result.token === "string" && result.token.trim() !== "") {
            ...
            if (savedToken === result.token) {
              ...
              console.error(
                "⚠️ Verifikasi: Token tidak tersimpan dengan benar!",
              );
              console.log(
                "Saved token length:",
        ...
        if (result.user) {
          ...
          console.log(
            "User data dengan role berhasil disimpan:",
            userDataWithRole,
          );
        }
        ...
        try {
          window.dispatchEvent(
            new CustomEvent("bersekolah:login-success", {
              detail: {
        ...
        setTimeout(() => {
          ...
          if (role === "admin" || role === "superadmin") {
            ...
            console.log(
              `Melakukan redirect ke Dashboard Admin sebagai ${role}`,
            );
          } else {
            redirectUrl = "/form-pendaftaran";
            console.log(
              "Melakukan redirect ke Form Pendaftaran sebagai user biasa",
            );
          }
        ...
        toast({
          title: "Login gagal",
          description:
            generalError || result.message || "Email atau password salah",
          variant: "destructive",
          duration: 5000,
        });
      }
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        ...
        setGeneralError(
          "Server mengembalikan data dalam format yang tidak valid. Pastikan API mengembalikan JSON yang valid.",
        );
      } else {
        setGeneralError(
