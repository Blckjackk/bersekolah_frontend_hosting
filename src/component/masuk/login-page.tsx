...
const LOGIN_HERO_IMAGE = "/assets/image/static/auth/image_login_hero.webp";
const LOGIN_HERO_FALLBACK =
  "/assets/image/company-profile/program-beasiswa.jpg";
  ...
  useEffect(() => {
    ...
    const checkAuthStatus = () => {
      ...
      if (token && user) {
        ...
        if (
          previousPage &&
          previousPage.indexOf("/masuk") === -1 &&
          previousPage.indexOf("/daftar") === -1
        ) {
      ...
      <div className="hidden relative bg-muted lg:block">
        <img
          ...
          onError={(e) => {
            ...
            if (target.src.indexOf(LOGIN_HERO_FALLBACK) === -1) {
              target.src = LOGIN_HERO_FALLBACK;
            }
          }}
        />
        ...
        <div className="absolute bottom-8 left-8 max-w-md text-white">
          ...
          <p>
            Berikan kesempatan pendidikan terbaik untuk mereka yang membutuhkan
            melalui program beasiswa Bersekolah.
          </p>
        </div>
      </div>
    </div>
